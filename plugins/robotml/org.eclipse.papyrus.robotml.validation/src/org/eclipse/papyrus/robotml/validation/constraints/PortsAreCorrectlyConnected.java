...
//	 * @return the boolean
//	 */
//	public Boolean ISConnectionBetweenSubSystems(ConnectableElement connectorEndRole0, ConnectableElement connectorEndRole1) {
...
//			if((connectorEndRole0.getOwner() instanceof Classifier) && (connectorEndRole1.getOwner() instanceof Classifier)) {
//				//3. verify that the connection is between sub systems that are contained in the same system
		...
		if(cends != null) {//if1
			...
			if(cends.size() == 2) {//if2
				if((cends.get(0) instanceof ConnectorEnd) && (cends.get(1) instanceof ConnectorEnd)) {//if3
					//verify that the connection is between two ports of two subsystems
					...
					if((connectorEndRole0 instanceof Port) && (connectorEndRole1 instanceof Port)) {//if4
						//verify that the connection is between two ports of two subsystems of the same containing system
						
							if(!(connectorEndRole0.getAppliedStereotypes().isEmpty()) && !(connectorEndRole1.getAppliedStereotypes().isEmpty())) {//if5
								//verify if the applied stereotype on the ports is the same
								...
								if(steretypesPort0.get(0).getName().equals(steretypesPort1.get(0).getName())) {//if8
									//verify that connections are set between ports of the same type
									...
									//here I have to check that the connection is between ports of sub systems and not between a port of the container system and a subsystem port's.	
									Boolean areSubsystems = true;
									
									if( (elt.getOwner() == connectorEndRole0.getOwner()) ||  (elt.getOwner() == connectorEndRole1.getOwner()) ){
										areSubsystems = false;
									}
								//System.err.println("\n\n\nConnector Owner: " + elt.getOwner()+ "\nPort0 owner: "+ connectorEndRole0.getOwner()+ "\nPort1 owner: "+ connectorEndRole1.getOwner()+" "+areSubsystems+"\n\n");
								//verify if connections are set between in and out DataFlowPorts or provided and required ServicePorts
									DataFlowPort flowport0 = ElementUtil.getStereotypeApplication(connectorEndRole0, DataFlowPort.class);
									DataFlowPort flowport1 = ElementUtil.getStereotypeApplication(connectorEndRole1, DataFlowPort.class);
									...
									ServicePort serviceport0 = ElementUtil.getStereotypeApplication(connectorEndRole0, ServicePort.class);
									ServicePort serviceport1 = ElementUtil.getStereotypeApplication(connectorEndRole1, ServicePort.class);
